...
void ofApp::draw(){
    ...
    if( draw_flag ){
        ...
        for(i = 0; i < num_of_dot; i++)
        {
            ...
            ofDrawCircle(Dots[i].x1, Dots[i].y1, dot_diameter / 2);
        }
...
void ofApp::keyReleased(int key){
    ...
    if (key == OF_KEY_RIGHT){
        ...
        cout << "Selcted Dot Coordinate is (" << Dots[cur].x1 << ", " << Dots[cur].y1 << ")" << endl;
    }
    if (key == OF_KEY_LEFT){
        cur--;
        if(cur < 0)
    ...
    */
    ...
    for (ofBuffer::Line it = buffer.getLines().begin(), end = buffer.getLines().end(); it != end; ++it) {
        ...
        if( words.size() == 1){
            ...
            int x1,y1,x2,y2;
            if(Ck){ // Input for actual information of lines
